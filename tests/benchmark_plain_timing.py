"""
Plain Intersection Benchmark - Timing Test
===========================================
Measures execution time of plain bitwise AND intersection
for different array sizes (baseline comparison).

Usage:
    python tests/benchmark_plain_timing.py
    python tests/benchmark_plain_timing.py --sizes 8 16 32 --runs 50
...
# Import plain intersection
from tests.plain_intersection import plain_intersection


class PlainBenchmark:
    """Benchmark plain AND intersection timing."""
    
    def __init__(self, array_sizes: List[int] = None):
        """
        Initialize plain benchmark.
    ...
    def benchmark_single(self, array_a: List[int], array_b: List[int], runs: int = 100) -> Dict:
        """
        Benchmark plain intersection for given arrays.
        ...
        for i in range(runs):
            ...
            result = plain_intersection(array_a, array_b)
    ...
    def run_benchmark(self, custom_arrays: Dict[int, Tuple[List[int], List[int]]] = None,
        ...
        """
        print("=" * 80)
        print("PLAIN INTERSECTION BENCHMARK (Baseline)")
        print("=" * 80)
        print(f"Algorithm:    Bitwise AND (no privacy)")
    ...
    def export_results(self, filename: str = "plain_timing_results.json"):
        """
    ...
    def export_csv(self, filename: str = "plain_timing_results.csv"):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark plain intersection timing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python tests/benchmark_plain_timing.py
  python tests/benchmark_plain_timing.py --sizes 8 16 32 --runs 50
  python tests/benchmark_plain_timing.py --pattern random
        """
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="plain_timing_results",
        help="Output filename prefix (default: plain_timing_results)"
    )
    
    args = parser.parse_args()
    
    # Create and run benchmark
    benchmark = PlainBenchmark(array_sizes=args.sizes)
    ...
    print()
    print("✅ Plain intersection benchmark complete!")
